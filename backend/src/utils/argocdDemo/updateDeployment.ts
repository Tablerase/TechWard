...
const GITHUB_BOT_PERSONAL_ACCESS_TOKEN = process.env.GITHUB_BOT_PERSONAL_ACCESS_TOKEN;
const GITHUB_REPO_NAME = process.env.GITHUB_REPO_NAME;
const GITHUB_REPO_OWNER = process.env.GITHUB_REPO_OWNER;

if (!GITHUB_BOT_PERSONAL_ACCESS_TOKEN || !GITHUB_BOT_USERNAME || !GITHUB_REPO_NAME || !GITHUB_REPO_OWNER) {
  throw new Error("Missing required environment variables for Argo Demo in .env")
}
...
const LOCAL_PATH = path.resolve("./tmp-repo");
 ...
 */
export async function updateDeployment(newTag: string = '1.25.0'): Promise<void> {
  if (!allowedTags.has(newTag)) {
    throw new Error(`Tag ${newTag} is not allowed. allowedTags are: ${[...allowedTags].join(',')}`)
  }

  const git: SimpleGit = simpleGit();
  if (!fs.existsSync(LOCAL_PATH)) {
    console.log("Cloning argo app repository...")
    await git.clone(REPO_URL, LOCAL_PATH)
  } else {
    await git.cwd(LOCAL_PATH)
    await git.pull("origin", "main")
  }

  const deploymentFile = path.join(LOCAL_PATH, "manifests/deployment.yaml")
  ...
  if (!doc.hasIn(containerPathInDoc)) throw new Error("No container found in deployment file")
  const imagePathInDoc = ["spec", "template", "spec", "containers", 0, "image"]
  if (!doc.hasIn(imagePathInDoc)) throw new Error("No image found in deployment container")
  ...
  await git.addConfig("user.email", `${GITHUB_BOT_USERNAME}@users.noreply.github.com`);
